...
At first, we just let the home page display the article data of the first page. If the corresponding parameters are passed in when visiting the home page, then we should display the corresponding content according to the parameters.
If the browser enters the homepage URL: [http://127.0.0.1:8080/](http://127.0.0.1:8080/), then we display the article data on the first page
...
## 一 Homepage function extension development
### 1.1 Modify the homepage logic
#### 1.1.1 Modify controller logic
...
```
#### 1.1.2 Functional expansion logic analysis
We first receive the tag value and the page value:
...
If it is the first case, then we should search by tag. If it is the second case, query according to the number of pages. If it is the third type, it will query by pagination by default, just query the first page.
### 1.2 Model layer logic implementation
#### 1.2.1 Logic Analysis
Next, let's implement the query according to the tag. There is a complicated place here, that is, we are writing the article. There may be one or more tags. If there are more than one, we connect according to &, for example : Go&socket. Then when searching by label, there are 4 situations:
```
Query the data of the homepage by label
There are four situations
1. There are ampersands and other symbols on the left and right sides
2. There are ampersands and other symbols on the left, and no symbols on the right
3. There are ampersands and other symbols on the right, and no symbols on the left
...
You can see the three articles with the web tags queried, re-enter the URL: [http://127.0.0.1:8080?tag=http](http://127.0.0.1:8080?tag=http) under the http tag Three articles were also queried.
...
In the previous section, we learned about the expansion of the home page. In this lesson, we continue to develop and implement new functions-file upload and image display. In the actual case, a photo album display function is provided. The pictures displayed by the photo album function need to be uploaded by files, and we will develop and implement them.
### 2.1 Create a data table
First, we need to create a data table in the database. To store pictures, we need the id of the picture, the path location of the picture, the name of the picture, and so on. In the mysqlUtil.go file under the utils toolkit, add a method to create a data table:
```go
//--------image--------
...
import (
"myblog/models"
"GitHub.com/open tracing/open tracing-go/log"
)
...
<input type="button" id="album-upload-button" value="Submit file">
</form>
</div>
</body>
</html>
...
//File upload to store file data through Formdata
...
## Three About me function development and project summary
The content of this lesson let's develop and complete the last function of this series of courses-the functional development of the About Me page, and at the same time make a review and summary of the content of this series of courses.
...
```
### 3.2 Course Summary
After 16 sessions of course content, we have completed the development of a blog system using the Beego framework. Through this project, we hope that everyone can master the use of Beego framework. Now let's review some important knowledge points that the Beego framework needs to master to develop a web project.
...
The use of Beego, a project management tool of the Beego framework, can facilitate developers to manage, debug, package projects, and automatically generate project directory structures.
...
* Custom routing: the developer specifies the routing resolution method
* **Session processing:**
